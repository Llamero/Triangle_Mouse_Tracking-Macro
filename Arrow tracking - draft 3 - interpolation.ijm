		...
		if(a>0){
			...
			for(b=a; b<rC.length; b++){
				...
				if(rC[b] != 0){
					...
					count = 1;
					for(c=a; c<b; c++){
						xC[c] = round((xC[b] - xC[a-1])*count/steps + xC[a-1]);
						yC[c] = round((yC[b] - yC[a-1])*count/steps + yC[a-1]);
						xT[c] = round((xT[b] - xT[a-1])*count/steps + xT[a-1]);
						yT[c] = round((yT[b] - yT[a-1])*count/steps + yT[a-1]);
						count += 1;
					}
